<?php

namespace Ss\Http;

class Session
{
    private $handler = null;
    private $request;
    private $response;
    private $isStart = false;
    private $sid = null;
    private $sessionName = 'SsSession';
    private $savePath;
    private $data = [];

    public function __construct(Request $request, Response $response, $handler = null)
    {
        $this->request = $request;
        $this->response = $response;
        $this->handler = $handler ?: new SessionHandler();
    }

    public function savePath(string $path = null)
    {
        if ($path) {
            if (!$this->isStart) {
                $this->savePath = rtrim($path, '/');

                return true;
            } else {
                return false;
            }
        } else {
            return $this->savePath;
        }
    }

    public function sid(string $sid = null)
    {
        if ($sid) {
            if (!$this->sid) {
                $this->sid = $sid;

                return true;
            } else {
                return false;
            }
        } else {
            return $this->sid;
        }
    }

    public function name(string $sessionName = null)
    {
        if ($sessionName) {
            if (!$this->isStart) {
                $this->sessionName = $sessionName;

                return true;
            } else {
                return false;
            }
        } else {
            return $this->sessionName;
        }
    }

    /*
     * 注意，这里并不是同步写入。write close的时候，才真实写入（与php一致）。
     */
    public function set($key, $val): bool
    {
        if ($this->isStart) {
            if (is_null($val)) {
                unset($this->data[$key]);
            } else {
                $this->data[$key] = $val;
            }

            return true;
        } else {
            return false;
        }
    }

    public function exist($key)
    {
        if ($this->isStart) {
            return isset($this->data[$key]);
        } else {
            return false;
        }
    }

    public function get($key)
    {
        if (isset($this->data[$key])) {
            return $this->data[$key];
        } else {
            return null;
        }
    }

    /*
     * 根据规范，session_destroy() 销毁当前会话中的全部数据，
     * 但是不会重置当前会话所关联的全局变量， 也不会重置会话 cookie。 如果需要再次使用会话变量，
     *  必须重新调用 session_start() 函数。
     */
    public function destroy()
    {
        if ($this->isStart) {
            $this->data = [];
            $this->handler->destroy($this->sid);

            return true;
        } else {
            return false;
        }
    }

    public function writeClose()
    {
        if ($this->isStart) {
            $this->isStart = false;
            if (!$this->handler->write($this->sid, serialize($this->data))) {
                trigger_error("save session {$this->sessionName}@{$this->sid} fail");
            }
            $this->handler->close();
            $this->resetStatus();
        }
    }

    public function start(): bool
    {
        if (!$this->isStart) {
            $this->isStart = $this->handler->open($this->savePath, $this->sessionName);
            if (!$this->isStart) {
                trigger_error("session open {$this->savePath}@{$this->sessionName} fail");

                return false;
            } else {
                //开启成功，则准备sid;
                $this->sid = $this->generateSid();
                //载入数据,实现原则中，start后则对Session文件加锁
                $data = $this->handler->read($this->sid);
                if (!empty($data)) {
                    $data = unserialize($data);
                    if (is_array($data)) {
                        $this->data = $data;
                    }
                }

                return true;
            }
        }

        return true;
    }

    private function generateSid(): string
    {
        $sid = $this->request->cookie($this->sessionName);
        if (!empty($sid)) {
            return $sid;
        } else {
            $sid = md5(microtime(true).$this->request->request()->fd);
            $this->response->cookie($this->sessionName, $sid);

            return $sid;
        }
    }

    public function __destruct()
    {
        $this->writeClose();
    }

    private function resetStatus()
    {
        $this->sid = null;
        $this->sessionName = 'SsSession';
        $this->savePath = null;
        $this->data = [];
    }
}
